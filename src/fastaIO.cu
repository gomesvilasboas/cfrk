...
Seq *ReadFasta(char *fileName, lint *nS)
{
  FILE *fastaFile;
  char *line = NULL, *aux;
  size_t len = 0;
  ...
  *nS = GetNs(fileName);
  seq = (Seq*)malloc(sizeof(Seq) * *nS);

  if ((fastaFile = fopen(fileName, "r")) == NULL) exit(EXIT_FAILURE);

  while ((getline(&line, &len, fastaFile)) != -1)
  {
    if (line[0] == '>')
    {
      count++;
...
//-------------------------------------------------------------------------------------------
void ProcessData(Seq *seq, struct read *rd, lint nN, lint nS, ushort flag)
{
